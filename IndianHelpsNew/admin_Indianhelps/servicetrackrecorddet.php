      ...
      <div class="card mb-3">
        ...
        <div class="card-body">
          <div class="table-responsive">
            ...
            <?php
              include"dbcon.php";
              $query="select * from service_trackrecord ";
              $result=mysqli_query($con,$query) or die(mysqli_error($con)) ;
              while($row=mysqli_fetch_array($result))
              {
                $service = $row['service_id'];
                $query1 = " select * from task_create where tid ='$service'";
                $result1= mysqli_query($con,$query1) or die(mysqli_error($con));
                $row1 = mysqli_fetch_array($result1);

                $user= $row['service_provider_id'];
                $query2 = "select * from register where rid='$user'";
                $result2=mysqli_query($con,$query2)or die(mysqli_error($con));
                $row2 = mysqli_fetch_array($result2);

                $helper= $row['helper_id'];
                $query3="select * from register where rid='$helper'";
                $result3=mysqli_query($con,$query3) or die(mysqli_error($con));
                $row3=mysqli_fetch_array($result3);
                  ?>
              <tr>
                   <td><?php echo $row1["tid"];?></td>
                <td><?php echo $row1["task_date"];?></td>
                <td><?php echo $row1["task_time"];?></td>
                <td><?php echo $row1["money"];?></td>
                <td><?php echo $row1["work_type"];?></td>
                <td><?php echo $row1["city"];?></td>
                <td><?php echo $row1["stats"];?></td>
                 	

                  <td><?php echo $row2["rid"];?></td>
                  <td><?php echo $row2["fname"];?></td>
                  <td><?php echo $row2["lname"];?></td>
                  <td><img src="../userimage/<?php echo $row2['fr_aadh_photo'];?>" height="100px" width="100px"/></td>
                  <td><?php echo $row2["mobile"];?></td>
                  <td><?php echo $row2["alt_mobile"];?></td>
